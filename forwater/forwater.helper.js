...
/*
	Imitate
	curl -X POST \
  		https://forwater.gov.in/api/api-manager/v1/consumer/mobile_app_openrap/credential/register \
...
let registerForwaterApp = () => {
	...
	let callableUrl = 'https://forwater.gov.in/api/api-manager/v1/consumer/mobile_app_openrap/credential/register'
	let headers = {
...
/*
	Must mimic
	curl -X POST \
  https://forwater.gov.in/api/api-manager/v1/consumer/mobile_device_openrap/credential/register \
...
*/
let registerForwaterDevice = (token) => {
	let defer = q.defer();
	const pattern = 'YYYY-MM-DD HH:mm:ss:SSSZZ';
	const timestamp = getTimestamp(pattern);
	let callableUrl = 'https://forwater.gov.in/api/api-manager/v1/consumer/mobile_device_openrap/credential/register'
	let headers = {
...
let generateOriginalJWTs = () => {
	...
	.then(value => {
		key = value.key;
		secret = value.secret;
		console.log("Forwater app registered");
		return generateJwt(key, secret);
	}).then(value => {
		token = value.token;
		return registerForwaterDevice(token);
	}).then(value => {
		key = value.key;
		secret = value.secret;
		console.log("Forwater device registered");
		return generateJwt(key, secret);
	}).then(value => {
        token = value.token;
